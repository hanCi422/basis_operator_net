...
class BasisONet(Basic_Model):
    ...
    def forward(self, x, y):
        ...
        score_in = _parralleled_inner_product_2d(x.unsqueeze(1).repeat((1, self.n_base_in, 1, 1)), \
            self.bases_in.unsqueeze(0).repeat((B_in, 1, 1)).reshape(B_in, self.n_base_in, J1_in, J2_in), self.h_in) # (B_in, n_base_in)
    ...
    def aec_in_forward(self, x):
        ...
        score_in = _parralleled_inner_product_2d(x.unsqueeze(1).repeat((1, self.n_base_in, 1, 1)), \
            self.bases_in.unsqueeze(0).repeat((B, 1, 1)).reshape(B, self.n_base_in, J1, J2), self.h_in)
    ...
    def check_orthogonality_in(self, J1, J2, path=None):
        ...
        for i in range(orth_matrix.shape[0]):
            for j in range(orth_matrix.shape[1]):
                orth_matrix[i, j] = _inner_product_2d(self.bases_in[i, :].reshape(J1, J2).unsqueeze(0), \
                    self.bases_in[j, :].reshape(J1, J2).unsqueeze(0), self.h_in).squeeze()
