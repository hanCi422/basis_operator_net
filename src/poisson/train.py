...
def main(**kwargs):
    ...
    if basis_name == 'sin':
        subpath = os.path.join(basis_name + '_' + str(opt.nbasis[0]) + '_' + str(opt.nbasis[1]))
    elif basis_name == 'grf':
    ...
    grid = np.load(os.path.join('datasets', subpath, 'grid.npy'))
    grid_in = grid.copy()
    grid_out = grid.copy()
...
if __name__ == "__main__":
    ...
    nbasis_out = 23
    length_scale = None
    main(model_name='BasisONet', basis_name = 'sin', sub = 1, \
